...
import { Search, SlidersHorizontal, Loader2, ArrowLeft, ArrowRight, GitPullRequest } from "lucide-react"
import { searchIssues, GithubIssue } from "@/core/services/github"
...
export function IssueExplorer() {
  ...
  const updateUrlParams = React.useCallback(
    (newParams: { q?: string; lang?: string; page?: number }) => {
  ...
  const handleLangSelect = (lang: string) => {
    const nextLang = langParam === lang ? "" : lang
    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-mono">
      <div className="border-4 border-foreground bg-black p-6 shadow-[6px_6px_0px_0px_rgba(255,255,255,1)] relative overflow-hidden bg-dot-pattern mb-8">
        ...
        <form onSubmit={handleSearchSubmit} className="flex gap-4">
          <div className="relative flex-grow border-2 border-foreground bg-black shadow-[3px_3px_0px_0px_rgba(255,255,255,1)] focus-within:shadow-[3px_3px_0px_0px_var(--primary)] transition-all">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-500" />
            <input
              type="text"
              value={searchInput}
              onChange={(e) => setSearchInput(e.target.value)}
              placeholder="Search by keywords inside issue titles (e.g. docker, fix, typos)..."
              className="w-full h-11 pl-10 pr-4 bg-transparent text-sm text-foreground focus:outline-none placeholder:text-zinc-600"
            />
          </div>
          <button
            type="submit"
            className="h-11 px-6 border-2 border-foreground bg-primary text-primary-foreground font-bold text-xs uppercase tracking-wider shadow-[3px_3px_0px_0px_rgba(255,255,255,1)] hover:bg-purple-600 active:translate-x-[1px] active:translate-y-[1px] cursor-pointer"
          >
            SEARCH
          </button>
        </form>
        ...
        <div className="lg:col-span-3 space-y-8">
          {isLoading ? (
            <div className="h-96 border-4 border-foreground bg-card flex flex-col items-center justify-center gap-3">
              ...
              <span className="text-xs uppercase text-zinc-500 font-bold">Searching Issues...</span>
            </div>
          ) : error ? (
            <div className="border-4 border-destructive bg-zinc-950 p-6 shadow-[4px_4px_0px_0px_var(--destructive)]">
              ...
              <button 
                onClick={() => updateUrlParams({ q: "", lang: "" })}
            ...
            <div className="h-96 border-4 border-foreground bg-card flex flex-col items-center justify-center gap-2">
              <span className="text-sm font-bold uppercase">No open issues found</span>
              <span className="text-xs text-zinc-500">Try adjusting your keyword filter or changing the language selection.</span>
            </div>
          ) : (
            <>
