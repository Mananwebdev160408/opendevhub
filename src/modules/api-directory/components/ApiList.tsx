...
import { Search, Code, Globe, ExternalLink } from "lucide-react"
...
export function ApiList({ initialApis }: ApiListProps) {
  const [query, setQuery] = React.useState("")
  const [activeCategory, setActiveCategory] = React.useState("ALL")
  const [apis] = React.useState<PublicApi[]>(initialApis)
  ...
  const categories = React.useMemo(() => {
    ...
    return ["ALL", ...sortedCats.slice(0, 7)]
  }, [apis])
